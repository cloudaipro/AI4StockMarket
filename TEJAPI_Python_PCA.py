...
data = tejapi.get("TWN/EWISAMPLE", idx_id="IX0002", start_date=mdate, paginate=True)
...
weight_port.iloc[0].T.sort_values(ascending=False).plot.bar(
    subplots=True, figsize=(20, 5), legend=False, sharey=True, ylim=(-0.75, 0.75)
)
...
def sharpe_ratio(ts_returns):
    ...
    if ts_returns.cumsum()[-1] < 0:
        annualized_return = (
            np.power(1 + abs(ts_returns.cumsum()[-1]) * 0.01, 1 / n_years) - 1
        ) * (-1)
    else:
        annualized_return = (
            np.power(1 + abs(ts_returns.cumsum()[-1]) * 0.01, 1 / n_years) - 1
...
n_components = 10
annualized_ret = np.array([0.0] * n_components)
sharpe_metric = np.array([0.0] * n_components)
annualized_vol = np.array([0.0] * n_components)
...
for i in range(n_components):
    ...
    eigen_port_returns = pd.Series(eigen_port_returns.squeeze(), index=X_train.index)
...
N = 5
result = pd.DataFrame(
    {"Annual Return": annualized_ret, "Vol": annualized_vol, "Sharpe": sharpe_metric}
)
...
def Backtest(i, data):
    ...
    eigen_port_returns = pd.Series(eigen_port_returns.squeeze(), index=data.index)
